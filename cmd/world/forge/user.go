...
func inputUserEmail(ctx context.Context, currentUserEmail string) (string, error) {
	maxAttempts := 5
	attempts := 0
...
func inputUserAvatarURL(ctx context.Context, currentUserAvatarURL string) (string, error) {
	attempts := 0
	maxAttempts := 5
	...
	for attempts < maxAttempts {
		...
		default:
			...
			if currentUserAvatarURL != "" {
				...
				fmt.Printf("\n✨ Enter new avatar URL (or press Enter to keep current): ")
			} else {
				fmt.Printf("\n✨ Enter avatar URL: ")
			}
