...
func (h *Handler) Switch(
	...
	projects, err := h.apiClient.GetProjects(ctx, h.configClient.GetConfig().OrganizationID)
	if err != nil {
	...
	if createNew {
		_, _, err = h.ProjectPreCreateUpdateValidation()
	...
	for {
		printer.NewLine(1)
		input, err := h.inputClient.Prompt(ctx, prompt, "")
		if err != nil {
