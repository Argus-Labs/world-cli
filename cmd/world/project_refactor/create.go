...
import (
	...
	"pkg.world.dev/world-cli/cmd/pkg/utils"
...
func (h *Handler) getSetupInput(ctx context.Context, project *models.Project, regions []string) error {
	// Get organization
	org, err := h.apiClient.GetOrganizationByID(ctx, h.configClient.GetConfig().OrganizationID)
	if err != nil {
...
func (h *Handler) inputName(ctx context.Context, project *models.Project) error {
	for {
		...
		name, err := h.inputClient.Prompt(ctx, "Enter project name", project.Name)
		if err != nil {
			return eris.Wrap(err, "Failed to get project name")
		}
...
func (h *Handler) inputSlug(ctx context.Context, project *models.Project) error {
	for {
		...
		default:
			...
			slug, err := h.inputClient.Prompt(ctx, "Slug", project.Slug)
			if err != nil {
				return eris.Wrap(err, "Failed to get project slug")
			}
...
func (h *Handler) inputRepoURLAndToken(ctx context.Context, project *models.Project) error {
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			repoURL, err := h.inputClient.Prompt(ctx, "Enter Repository URL", project.RepoURL)
			if err != nil {
...
func (h *Handler) promptForRepoToken(ctx context.Context, project *models.Project) (string, error) {
	...
	repoToken, err := h.inputClient.Prompt(ctx, "\nEnter Token", project.RepoToken)
	if err != nil {
		return "", eris.Wrap(err, "Failed to get repository token")
	}
...
func (h *Handler) inputRepoPath(ctx context.Context, project *models.Project) {
	// Get repository Path
	for {
		repoPath, err := h.inputClient.Prompt(ctx, "Enter path to Cardinal within Repo (Empty Valid)", project.RepoPath)
		if err != nil {
			printer.Errorf("%v\n", err)
			printer.NewLine(1)
			continue
...
func (h *Handler) promptEnableNotifications(
	...
	default:
		...
		return h.inputClient.Confirm(ctx, prompt, "n")
	}
}
...
func (h *Handler) promptForToken(
	...
	default:
		...
		token, err := h.inputClient.Prompt(ctx, prompt, "")
		if err != nil {
...
func (h *Handler) promptForChannelID(ctx context.Context, serviceName string) (string, error) {
	...
	default:
		...
		channelID, err := h.inputClient.Prompt(ctx, prompt, "")
		if err != nil {
...
func (h *Handler) inputAvatarURL(ctx context.Context, project *models.Project) error {
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			avatarURL, err := h.inputClient.Prompt(ctx, "Enter avatar URL (Empty Valid)", project.AvatarURL)
			if err != nil {
