...
func (h *Handler) Create(ctx context.Context, _ *models.CommandState, flags models.CreateOrganizationFlags,
	...
	for {
		...
		for {
			orgName, err = h.inputClient.Prompt(ctx, "Enter organization name", flags.Name)
			if err != nil {
		...
		for {
			...
			orgSlug, err = h.inputClient.Prompt(ctx, "Enter organization slug", orgSlug)
			if err != nil {
		...
		for {
			orgAvatarURL, err = h.inputClient.Prompt(
				ctx,
		...
		// Get confirmation
		printer.NewLine(1)
		confirm, err := h.inputClient.Confirm(ctx, "Create organization with these details? (Y/n)", "n")
		if err != nil {
