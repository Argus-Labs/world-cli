...
func (s *Service) SetupCommandState(ctx context.Context, req models.SetupRequest) (*models.CommandState, error) {
	result := &models.CommandState{}

	cfg := s.handleConfig()

	// Handle login step
	if err := s.handleLogin(ctx, req.LoginRequired, result, cfg); err != nil {
		return result, err
	}

	// Handle organization invitations if logged in
	if result.LoggedIn && req.LoginRequired != models.IgnoreLogin {
		if err := s.handleOrganizationInvitations(ctx); err != nil {
			return result, err
		}
	}

	// Handle repository lookup if needed
	if err := s.handleRepoLookup(ctx, req, result, cfg); err != nil {
		return result, err
	}

	// Handle state setup
	if err := s.handleStateSetup(ctx, req, result, cfg); err != nil {
		return result, err
	}

	// Save config changes
	if err := s.configClient.Save(); err != nil {
...
func (s *Service) handleConfig() *config.Config {
	cfg := s.configClient.GetConfig()
	if cfg == nil {
	...
	cfg.CurrRepoPath, cfg.CurrRepoURL, _ = s.repoClient.FindGitPathAndURL()
	if cfg.CurrRepoURL != "" {
...
func (s *Service) handleOrganizationInvitations(ctx context.Context) error {
	orgs, err := s.apiClient.GetOrganizationsInvitedTo(ctx)
	if err != nil {
...
func (s *Service) handleLogin(
	...
	if loginReq == models.NeedLogin && loggedIn {
		user, err := s.apiClient.GetUser(ctx)
		if err != nil {
			return err
		}
		result.User = &user
...
func (s *Service) handleRepoLookup(
	ctx context.Context,
	...
	if needRepoLookup {
		...
		project, err := s.apiClient.LookupProjectFromRepo(ctx, cfg.CurrRepoURL, cfg.CurrRepoPath)
		if err != nil {
			return eris.Wrap(err, "failed to lookup project from git repo")
		}

		if project.ID != "" {
			s.configClient.AddKnownProject(
				project.ID,
				project.Name,
				project.OrgID,
				cfg.CurrRepoURL,
...
func (s *Service) handleStateSetup(
	ctx context.Context,
	req models.SetupRequest,
	result *models.CommandState,
	cfg *config.Config,
	...
	if s.inKnownRepo(ctx, cfg, result) {
	...
	if haveOrgID && needOrgIDOnly {
		...
		case models.NeedData, models.NeedIDOnly:
			if err := s.handleNeedOrgData(ctx, result, cfg); err != nil {
				return err
			}
		case models.NeedExistingData, models.NeedExistingIDOnly:
			if err := s.handleNeedExistingOrgData(ctx, result, cfg); err != nil {
				return err
			}
		}
	}
	...
	if haveProjectID && needProjectIDOnly {
		...
		case models.NeedData, models.NeedIDOnly:
			if err := s.handleNeedProjectData(ctx, result, cfg); err != nil {
				return err
			}
		case models.NeedExistingData, models.NeedExistingIDOnly:
			if err := s.handleNeedExistingProjectData(ctx, result, cfg); err != nil {
				return err
			}
		}
	}
...
func (s *Service) inKnownRepo(ctx context.Context, cfg *config.Config, result *models.CommandState) bool {
	if cfg.CurrRepoKnown {
		org, err := s.apiClient.GetOrganizationByID(ctx, cfg.OrganizationID)
		if err != nil {
			return false
		}
		proj, err := s.apiClient.GetProjectByID(ctx, cfg.ProjectID)
		if err != nil {
			return false
		}
